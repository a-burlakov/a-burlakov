_just a draft yet ⚒_

So now I'm web developer junior

Before I spent 8 years on 1C (russian SAP or Salesforce for cheap and rapid enterprise developing). Made [voluntary projects](https://www.ekbmiloserdie.ru/news/item_03807-zachem_vesti_bazu_dannyih_lits_bomj), mobile applications, medicine tools, high load, >20 TB databases, queryes to > 1 billion bases.

<div align="center">
blocks: site , mail, vk, telegram, linkedin
</div>


### Tools I use
python django postgres sqlite mssql vue docker nginx gunicorn pycharm windows ubuntu githubactions git chrome postman books

<img align="right" src="./img/hydra_fight.gif" height=225 />

### Recently on my blog (rus)
* 🔥 **[Победа, смог поставить SSL! 🔐](https://aaburlakov.ru/blog/pobeda-smog-postavit-ssl/)** <br> _Это было не сложно, но вот NGINX..._
* 🔥 **[Настроил обновление README.md с помощью DRF!](https://aaburlakov.ru/blog/ura-poluchilos-obnovit-readmemd/)** <br> _С GitHub Actions на /api/v1/recentarticles_
* **[Ура! Получилось!](https://aaburlakov.ru/blog/ura-poluchilos/)** <br> _Задеплоил свой сайт через Docker! 🐳_

read more at [aaburlakov.ru](https://aaburlakov.ru)

<div align="center">

![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=a-burlakov&show_icons=true&hide_border=true&hide=contribs&hide_title=True)

</div>